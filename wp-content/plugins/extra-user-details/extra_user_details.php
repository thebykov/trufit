...
function eud_plugin_menu()
{
    $eud_page = add_submenu_page(
        'users.php', 'Extra User Details Options', 'Extra User Details', 'edit_users', 'extra-user-details',
        'eud_plugin_options'
    );
    ...
    if (isset($_POST['eud_submit'])) {
        ...
        if (count($error_fields)) {
            echo '<div class="error">
                    <p><strong>' . __( 'However, highlighted fields are not updated because contain errors! Please correct them and update fields again or these fields will be lost.', 'extra-user-details' ) . '</strong></p>
                </div>';
        }
    }
        ...
        jQuery(document).ready(function() {
            ...
            jQuery('[name="eud_submit"]').click(function() {
                ...
                if (duplicate_slugs.length) {
                    alert('<?php _e('Duplicate fields detected. Please fix highlighted.', 'extra-user-details'); ?>');
