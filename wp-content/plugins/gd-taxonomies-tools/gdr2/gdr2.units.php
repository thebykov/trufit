...
if (!class_exists('gdr2_Units')) {
    ...
    class gdr2_Units {
        ...
        public function init() {
            $this->units = array(
                'angle' => array('source' => '__internal__', 'name' => __("Angle", "gdr2")), 
                'area' => array('source' => '__internal__', 'name' => __("Area", "gdr2")), 
                'brightness' => array('source' => '__internal__', 'name' => __("Brightness", "gdr2")), 
                'currency' => array('source' => '__internal__', 'name' => __("Currency", "gdr2")), 
                'electric_current' => array('source' => '__internal__', 'name' => __("Electric Current", "gdr2")), 
                'electrical_charge' => array('source' => '__internal__', 'name' => __("Electrical Charge", "gdr2")), 
                'energy' => array('source' => '__internal__', 'name' => __("Energy", "gdr2")),
                'frequency' => array('source' => '__internal__', 'name' => __("Frequency", "gdr2")),
                'fuel_consumption' => array('source' => '__internal__', 'name' => __("Fuel Consumption", "gdr2")), 
                'length' => array('source' => '__internal__', 'name' => __("Lenght or Distance", "gdr2")), 
                'memory' => array('source' => '__internal__', 'name' => __("Memory", "gdr2")), 
                'power' => array('source' => '__internal__', 'name' => __("Power", "gdr2")), 
                'sound' => array('source' => '__internal__', 'name' => __("Sound", "gdr2")), 
                'speed' => array('source' => '__internal__', 'name' => __("Speed", "gdr2")),
                'temperature' => array('source' => '__internal__', 'name' => __("Temperature", "gdr2")),
                'time' => array('source' => '__internal__', 'name' => __("Time", "gdr2")), 
                'volume' => array('source' => '__internal__', 'name' => __("Volume", "gdr2")),
                'weight' => array('source' => '__internal__', 'name' => __("Weight / Mass", "gdr2"))
            );
        }
        ...
        private function load_unit_type($name) {
            if (!isset($this->data[$name])) {
                ...
                $path = $unit['source'] == '__internal__' ? dirname(__FILE__).'/units/gdr2.unit.'.$name.'.php' : $unit['source'];

                if (file_exists($path)) {
                    include($path);
                    $this->data[$name] = $gdr2_unit_loading;
                }
            }
        }
...
if (!function_exists('gdr2_register_unit_type')) {
    /**
     * Register new unit type.
     * 
     * @param string $name name of the unit
     ...
     * @param string $source path to the unit definition file
     */
    function gdr2_register_unit_type($name, $title, $source) {
        global $gdr2_units;
        $gdr2_units->register_unit_type($name, $title, $source);
    }
}
...
if (!function_exists('gdr2_unit_is_valid')) {
    ...
    function gdr2_unit_is_valid($name, $unit) {
        global $gdr2_units;
        return $gdr2_units->is_unit_valid($name, $unit);
    }
}
