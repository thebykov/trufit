...
?>		
   ...
   <span>You can compress and try the compressed version of your images by switching directories.</span>
    	<ul>
			...
			$temp_path_exists = array_key_exists($temp_path_encoded, $dir_size['temp']);
			
			
			if(!$is_temp){
			
				$dir_listing[] = '<li class="wpsu_link_dir '.(wpsu_temp(base64_decode($k))?'wpsu_temp_text':'').'" data-linked="'.$k.'"><span title="Directory Name: '.ucwords(base64_decode($k)).'">'.ucwords($dir_path).'</span> (<span>'.wpsu_formatSizeUnits($fs).'</span>) ';
				if($temp_path_exists): 
				...
				$dir_listing[] = '- <span class="wpsu_temp_text" data-linked="'.$temp_path_encoded.'"><a title="Click here to switch directory" class="wpsu_ud button button-secondary">'.$version_type.' version ('.wpsu_formatSizeUnits($fs_temp).')</a></span>'; endif; 
				$dir_listing[] = '</li>';
